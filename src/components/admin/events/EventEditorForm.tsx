...
import { Bus, X, Users } from "lucide-react";
import type { EventCategoryRow, EventRow, EventStatus, SponsorRow } from "@/interfaces/events";
...
export const EventEditorForm = (props: EventEditorFormProps) => {
    ...
    <Tabs defaultValue="info" className="mt-2">
      ...
      <TabsContent value="info" className="space-y-4 pt-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">Vuoto = generato automaticamente. Univoco per sito.</p>
          </div>
          <div className="space-y-2">
            <Label htmlFor="category">Categoria</Label>
            <Select value={e?.category_id ?? "none"} onValueChange={(v) => set({ category_id: v === "none" ? null : v })}>
          ...
          <div className="space-y-2">
            ...
            {isCancelling && e?.id && <p className="text-xs text-destructive">L'evento sarà nascosto agli utenti. Le prenotazioni esistenti restano in DB.</p>}
          </div>
          <div className="space-y-2">
            ...
            {e?.id && (
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className={soldOut ? "text-destructive font-medium" : "text-muted-foreground"}>
                  {cap == null ? `${booked} iscritti (capienza illimitata)` : `${booked}/${cap} iscritti · ${remaining} posti liberi${soldOut ? " · SOLD OUT" : ""}`}
                </span>
              </div>
            )}
          </div>
        ...
        <div className="space-y-2">
          ...
          <Textarea id="short_description" rows={2} maxLength={280} value={e?.short_description ?? ""} onChange={(ev) => set({ short_description: ev.target.value })} />
        ...
        <div className="space-y-2">
          ...
          <Textarea id="description" rows={6} value={e?.description ?? ""} onChange={(ev) => set({ description: ev.target.value })} />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div className="space-y-2">
            ...
            {e?.ends_at && e?.starts_at && e.ends_at < e.starts_at && <p className="text-xs text-destructive">La data di fine deve essere successiva all'inizio.</p>}
          </div>
        </div>
      ...
      <TabsContent value="media" className="space-y-6 pt-4">
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
            ...
            />
            {e?.cover_image_url && <Button type="button" variant="outline" size="sm" className="w-full sm:w-auto" onClick={props.removeCoverImage}>Rimuovi</Button>}
          </div>
          {props.uploadingCover && <p className="text-xs text-muted-foreground">Caricamento in corso...</p>}
          {e?.cover_image_url && <img src={e.cover_image_url} alt="Anteprima" className="mt-2 w-full max-h-56 object-cover rounded-md border border-border" />}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {props.sponsors.map((s) => {
              ...
              return (
                ...
                >
                  {s.logo_url ? <img src={s.logo_url} alt={s.name} className="w-10 h-10 object-contain rounded bg-background" /> : <div className="w-10 h-10 rounded bg-muted" />}
      ...
      <TabsContent value="seo" className="space-y-4 pt-4">
        <div className="space-y-2">
          ...
          <Input id="seo_title" maxLength={70} value={e?.seo_title ?? ""} placeholder={e?.title ?? "Titolo per motori di ricerca"} onChange={(ev) => set({ seo_title: ev.target.value })} />
          <p className="text-xs text-muted-foreground">Consigliato 50-60 caratteri. Vuoto = usa il titolo evento. <span className={(e?.seo_title?.length ?? 0) > 60 ? "text-destructive" : ""}>{e?.seo_title?.length ?? 0}/70</span></p>
        </div>

        <div className="space-y-2">
          <Label htmlFor="seo_description">Meta description</Label>
          <Textarea id="seo_description" rows={3} maxLength={200} value={e?.seo_description ?? ""} placeholder={e?.short_description ?? "Descrizione breve per Google/social"} onChange={(ev) => set({ seo_description: ev.target.value })} />
          <p className="text-xs text-muted-foreground">Consigliato 120-160 caratteri. Vuoto = usa la descrizione breve. <span className={(e?.seo_description?.length ?? 0) > 160 ? "text-destructive" : ""}>{e?.seo_description?.length ?? 0}/200</span></p>
        </div>

        <div className="space-y-2">
          <Label htmlFor="og_image_url">OG Image (URL)</Label>
          <Input id="og_image_url" type="url" value={e?.og_image_url ?? ""} placeholder={e?.cover_image_url ?? "https://.../social-preview.jpg"} onChange={(ev) => set({ og_image_url: ev.target.value })} />
          <p className="text-xs text-muted-foreground">Immagine 1200x630 px per anteprime social. Vuoto = usa la copertina dell'evento.</p>
          {e?.og_image_url && <img src={e.og_image_url} alt="Anteprima OG" className="mt-2 w-full max-h-48 object-cover rounded-md border border-border" />}
        </div>

        <div className="rounded-md border border-border p-3 bg-muted/30">
          <p className="text-[11px] uppercase tracking-wide text-muted-foreground mb-1">Anteprima Google</p>
          <p className="text-base text-primary truncate">{e?.seo_title?.trim() || e?.title || "Titolo evento"}</p>
          <p className="text-xs text-emerald-600 truncate">stayup.it/eventi/{e?.slug || slugify(e?.title ?? "evento")}</p>
          <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{e?.seo_description?.trim() || e?.short_description || "Descrizione dell'evento..."}</p>
        </div>
      </TabsContent>
